...
fn test_template_rendering_in_http_action() {
    ...
    assert_eq!(
        rendered_url,
        "https://api.example.com/build/feature/test-branch"
    );
    ...
    headers.insert(
        ...
        "Bearer {{ env.TEST_API_TOKEN }}".to_string(),
    );
    headers.insert("Content-Type".to_string(), "application/json".to_string());

    let rendered_headers = template::render_template_map(&headers, &context);
    assert_eq!(
        rendered_headers.get("Authorization").unwrap(),
        "Bearer secret-token-123"
    );
    assert_eq!(
        rendered_headers.get("Content-Type").unwrap(),
        "application/json"
    );
